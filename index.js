...
const events = await downloadPagedArray('https://api.github.com/users/shubham-vunet/events', 'events.json');
...
const freshFollowers = await downloadPagedArray('https://api.github.com/users/shubham-vunet/followers', 'followers.dev.json');
...
const repositories = await downloadPagedArray('https://api.github.com/users/shubham-vunet/repos', 'repositories.dev.json');
...
for (const repository of repositories) {
  ...
  if (name !== 'shubham-vunet' && todos[name]?.stamp !== pushed_at) {
    const readme = todos[name]?.readme ?? 'readme.md';
    let content;

    // Download the readme at the remembered or default name
    try {
      content = await download(`https://api.github.com/repos/shubham-vunet/${name}/contents/${readme}`);
    ...
    // Download the readme at the alternate name or fail
      ...
      content = await download(`https://api.github.com/repos/shubham-vunet/${name}/contents/${oppositeReadme}`);
    }
    ...
    todos[name].todos = [];
    for await (const { text } of tx('.', name + '.' + readme)) {
...
for (const repository in _repositories) {
  ...
  for (const stamp in stats) {
    ...
    if (stat.stars !== _stat.stars && stamp?.localeCompare(cutoff) >= 0) {
      events.push({ actor: { login }, created_at: stamp, type: 'RepositoryEvent', repo: { name: 'shubham-vunet/' + repository }, payload: { action: 'starred', old: _stat.stars, new: stat.stars } });
    }

    if (stat.forks !== _stat.forks && stamp?.localeCompare(cutoff) >= 0) {
      events.push({ actor: { login }, created_at: stamp, type: 'RepositoryEvent', repo: { name: 'shubham-vunet/' + repository }, payload: { action: 'forked', old: _stat.forks, new: stat.forks } });
    }
...
const issuesAndPrs = await downloadPagedArray('https://api.github.com/search/issues?q=org:shubham-vunet+is:open', 'issues-and-prs.json');
...
const forkPrs = await downloadPagedArray('https://api.github.com/search/issues?q=is:pr+is:open+author:shubham-vunet+-org:shubham-vunet', 'fork-prs.json');
...
const forksMarkDown =
  forks.length === 0
    ...
    : forks.length === 1
      ...
      : `[${forks.length}${nbsp}forks${nbsp}🍴](https://github.com/shubham-vunet?tab=repositories&q=&type=fork)${identicalForks.length > 0 ? ' ᐧ ' : ''}`
  ;
...
const identicalForksMarkDown =
  identicalForks.length === 0
    ...
    : identicalForks.length === 1
      ? `\n[One${nbsp}identical${nbsp}fork:${nbsp}\`${identicalForks[0]}\`${nbsp}🍴⚠️](https://github.com/shubham-vunet/${identicalForks[0]})`
...
<img align="left" alt="shubham-vunet's GitHub Stats" src="https://github-readme-stats.codestackr.vercel.app/api?username=shubham-vunet&show_icons=true&hide_border=true" />


<div align="center">

<img src="https://github.com/shubham-vunet/shubham-vunet/actions/workflows/main.yml/badge.svg">

</div>

<div align="center">

[${followerCount}&nbsp;followers&nbsp;🤝](https://github.com/shubham-vunet?tab=followers) ᐧ
[${repositories.length}&nbsp;repositories&nbsp;📓](https://github.com/shubham-vunet?tab=repositories) ᐧ
